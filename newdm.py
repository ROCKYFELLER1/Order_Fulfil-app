...
def load_data(file):
    ...
    required_cols = ["SOLDTO", "Type", "Incoterm", "Status Summary", "ORDERED_QUANTITY"]
    ...
    group_cols = ["SOLDTO", "Type", "Incoterm"]
...
if st.button("Ask"):
    ...
    breakdown = customer_df.groupby(["Type", "Incoterm"], as_index=False).agg(
        {"Backlog": "sum", "MTD": "sum"}
    )
    ...
    if question_type in ["Backlog", "Backlog & MTD"]:
        ...
        backlog_text += "### Backlog by Type and Incoterm\n"
        for _, row in breakdown.iterrows():
            backlog_text += (
                f"- {row['Type']} | {row['Incoterm']} → {row['Backlog']:,.2f}\n"
            )
    ...
    if question_type in ["MTD", "Backlog & MTD"]:
        ...
        mtd_text += "### MTD by Type and Incoterm\n"
        for _, row in breakdown.iterrows():
            mtd_text += f"- {row['Type']} | {row['Incoterm']} → {row['MTD']:,.2f}\n"
